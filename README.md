...
- **Веб-интерфейс**: Дашборд `/errors` для анализа ошибок в реальном времени
...
### 🆕 Новые endpoints
- `GET /errors` - Веб-интерфейс анализа ошибок
- `GET /api/v1/errors` - API списка ошибок с фильтрацией
- `GET /api/v1/errors/stats` - Статистика ошибок  
- `GET /api/v1/errors/:id` - Детали конкретной ошибки
- `GET /test-error` - Генерация тестовой ошибки
